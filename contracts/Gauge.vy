...
interface Controller:
    ...
    def gauge_relative_weight(addr: bytes32, time: uint256) -> uint256: view
    def voting_escrow() -> address: view
    def checkpoint(): nonpayable
    def checkpoint_gauge(addr: bytes32): nonpayable
...
interface VotingEscrow:
    def balanceOf(addr: bytes32) -> uint256: view
    def user_point_epoch(addr: address) -> uint256: view
    def user_point_history__ts(addr: bytes32, epoch: uint256) -> uint256: view
...
BOOST_WARMUP: constant(uint256) = 0 # 2 * 7 * 86400
...
@external
def initial(lp_addr: address, _minter: address, _speeder: address, _controller: address):
    """
    ...
    crv_addr: address = _speeder
    self.crv_token = crv_addr
    #controller_addr: address = Minter(_minter).controller()
    self.controller = _controller
    self.voting_escrow = Controller(_controller).voting_escrow()
...
def _update_liquidity_limit(addr: address, l: uint256, L: uint256):
    ...
    voting_balance: uint256 = VotingEscrow(_voting_escrow).balanceOf(convert(addr,bytes32))
...
def _checkpoint(addr: address):
    ...
    Controller(_controller).checkpoint_gauge(convert(self.lp_token,bytes32))
    ...
    if block.timestamp > _period_time:
        ...
        for i in range(500):
            ...
            w: uint256 = Controller(_controller).gauge_relative_weight(convert(self.lp_token,bytes32), prev_week_time / WEEK * WEEK)
...
@external
def claimable_tokens_view(_addr: bytes32) -> uint256:
...
def kick(_addr: bytes32):
    ...
    t_ve: uint256 = VotingEscrow(_voting_escrow).user_point_history__ts(
        _addr, VotingEscrow(_voting_escrow).user_point_epoch(addr)
    )
    _balance: uint256 = self.balanceOf[addr]

    assert  VotingEscrow(_voting_escrow).balanceOf(convert(addr,bytes32)) == 0 or t_ve > t_last # dev: kick not allowed
...
@external
def set_approve_deposit(_addr: bytes32, can_deposit: bool):
    """
