...
def convert_to_addr(src: bytes32) -> address:
    ...
    return convert(convert(bitwise_and(REDUCER, convert(src, uint256)),bytes32), address)
...
def initial(
    ...
    @param _token Fee token address (TUSD)
...
@view
@external
def ve_for_at(user: bytes32, _timestamp: uint256) -> uint256:
    """
    ...
    """
    _user: address = self.convert_to_addr(user)
...
@external
@nonreentrant('lock')
def claim_many(_receivers: bytes32[20]) -> bool:
    """
