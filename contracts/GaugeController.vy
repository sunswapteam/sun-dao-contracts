...
interface VotingEscrow:
    def get_last_user_slope(addr: bytes32) -> int128: view
    def locked__end(addr: bytes32) -> uint256: view
...
@external
def initial(_token: bytes32, _voting_escrow: bytes32):
    """
    ...
    self.token = self.convert_to_addr(_token)
    self.voting_escrow = self.convert_to_addr(_voting_escrow)

    assert self.token != ZERO_ADDRESS
    assert self.voting_escrow  != ZERO_ADDRESS
...
@external
@view
def gauge_types(_addr: bytes32) -> int128:
    """
    @notice Get gauge type for address
    @param _addr Gauge address
    @return Gauge type id
    """
    gauge_type: int128 = self.gauge_types_[self.convert_to_addr(_addr)]
    assert gauge_type != 0
...
@external
def add_gauge(_addr: bytes32, gauge_type: int128, weight: uint256 = 0):
    """
...
@external
def checkpoint_gauge(_addr: bytes32):
    """
...
@external
@view
def gauge_relative_weight(addr: bytes32, time: uint256 = block.timestamp) -> uint256:
    """
    @notice Get Gauge relative weight (not more than 1.0) normalized to 1e18
            (e.g. 1.0 == 1e18). Inflation which will be received by it is
            inflation_rate * relative_weight / 1e18
    ...
    @return Value of relative weight normalized to 1e18
    """
    return self._gauge_relative_weight(self.convert_to_addr(addr), time)


@external
def gauge_relative_weight_write(_addr: bytes32, time: uint256 = block.timestamp) -> uint256:
    """
...
@external
def change_gauge_weight(addr: bytes32, weight: uint256):
    """
    ...
    """
    assert msg.sender == self.admin
    self._change_gauge_weight(self.convert_to_addr(addr), weight)


@external
def vote_for_gauge_weights(_gauge_addr_bytes32: bytes32, _user_weight: uint256):
    """
    @notice Allocate voting power for changing pool weights
    @param _gauge_addr Gauge which `msg.sender` votes for
    @param _user_weight Weight for a gauge in bps (units of 0.01%). Minimal is 0.01%. Ignored if 0
    """
    _gauge_addr: address = self.convert_to_addr(_gauge_addr_bytes32)

    escrow: address = self.voting_escrow
    slope: uint256 = convert(VotingEscrow(escrow).get_last_user_slope(convert(msg.sender,bytes32)), uint256)
    lock_end: uint256 = VotingEscrow(escrow).locked__end(convert(msg.sender,bytes32))
...
@external
@view
def get_gauge_weight(_addr: bytes32) -> uint256:
    """
